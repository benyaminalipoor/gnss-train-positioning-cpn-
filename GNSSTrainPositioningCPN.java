...
public class GNSSTrainPositioningCPN {
    ...
    static class PositionSolutionCPN {
        ...
        TrainState predict(TrainState state) {
            ...
            double[][] F = Matrix.identity(6);
            for (int i = 0; i < 3; i++) {
                F[i][i+3] = dt; // Position updated by velocity
            }
            
            // Process noise covariance Q
            double[][] Q = new double[6][6];
            for (int i = 0; i < 3; i++) {
                Q[i][i] = processNoise * dt * dt / 4.0;      // Position noise
                Q[i+3][i+3] = processNoise * dt;             // Velocity noise
                Q[i][i+3] = processNoise * dt * dt / 2.0;    // Cross terms
                Q[i+3][i] = processNoise * dt * dt / 2.0;
            }
            
            // Predict covariance: P = F*P*F' + Q
            double[][] FP = Matrix.multiply(F, state.covariance);
            double[][] FPFt = Matrix.multiply(FP, Matrix.transpose(F));
            predicted.covariance = Matrix.add(FPFt, Q);
        ...
        TrainState update(TrainState predicted, List<GNSSSignal> signals) {
            ...
            double[][] I_KH = Matrix.subtract(Matrix.identity(6), KH);
    ...
    static class SimulationResult {
        ...
        double getMaxError() {
            return errors.stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
        }
