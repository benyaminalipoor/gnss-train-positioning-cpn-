...
class ExtendedKalmanFilter:
    ...
    def __init__(self, initial_position: Position, initial_velocity: float = 20.0):
        ...
        # Pre-allocate matrices for EKF update (performance optimization)
        self._h = np.zeros(4)  # predicted measurements
        self._H = np.zeros((4, 8))  # Jacobian matrix
        self._z = np.zeros(4)  # measurement vector
        self._I8 = np.eye(8)  # identity matrix for state update
    ...
    def update(self, signals: List[GNSSSignal], measurement_noise: float = 1.0):
        ...
        # Use first 4 signals for position solution (avoid creating new list)
        num_signals = 4
        
        # Measurement vector (pseudoranges) - reuse pre-allocated array
        for i in range(num_signals):
            self._z[i] = signals[i].psr
        
        # Reset pre-allocated matrices
        self._h.fill(0)
        self._H.fill(0)
        ...
        for i in range(num_signals):
            ...
            self._h[i] = r + self.state[6]
            
            # Jacobian matrix
            if r > 0:
                self._H[i, 0] = -dx / r  # ∂h/∂x
                self._H[i, 1] = -dy / r  # ∂h/∂y
                self._H[i, 2] = -dz / r  # ∂h/∂z
                self._H[i, 6] = 1.0      # ∂h/∂clk_bias
        
        # Innovation
        y = self._z - self._h
        
        # Measurement noise covariance
        R = np.eye(num_signals) * measurement_noise
        ...
        # Update covariance (reuse identity matrix)
        self.P = (self._I8 - K @ self._H) @ self.P
...
class GNSSSimulator:
    ...
    def apply_environment_scenario(self, signals: List[GNSSSignal], scenario: EnvironmentScenario,
        ...
        if scenario == EnvironmentScenario.OPEN_AREA:
            ...
            noise_factor = 4.2
            # Vectorized random signal degradation for performance
            if len(signals) > 0:
                random_vals = np.random.random(len(signals))
                noise_vals = np.random.normal(0, 0.8, len(signals))
                for i, sig in enumerate(signals):
                    if random_vals[i] < 0.10:  # 10% chance of partial blockage
            ...
            if epoch < tunnel_length:
                ...
                # Vectorized noise application for performance
                if len(signals) > 0:
                    noise_vals = np.random.normal(0, noise_factor * 1.0, len(signals))
                    for i, sig in enumerate(signals):
                        sig.psr += noise_vals[i]
                return signals, noise_factor
            else:
