...
class GNSSSignalGenerator:
    ...
    def generate_test_signals(self, time_step: int, num_satellites: int = 8) -> List[Signal]:
        """Generate test GNSS signals for visible satellites"""
        signals = []
        
        for i in range(num_satellites):
            # Generate satellite position (approximate orbital parameters)
            orbit_radius = 20200000 + random.uniform(-1000000, 1000000)  # ~20,200 km
            ...
            geometric_range = np.sqrt((sat_x - recv_x)**2 + 
                                     (sat_y - recv_y)**2 + 
                                     (sat_z - recv_z)**2)
...
class InterferenceGenerator:
    ...
    def am_interference(self, time: float, signals: List[Signal]) -> Tuple[List[Signal], float]:
        """
        ...
        carrier = np.sin(2 * np.pi * self.L1_FREQUENCY * time)
        envelope = 1 + modulation_depth * np.sin(2 * np.pi * envelope_freq * time)
    ...
    def fm_interference(self, time: float, signals: List[Signal]) -> Tuple[List[Signal], float]:
        """
        ...
        interference = (np.sin(2 * np.pi * freq1 * time) + 
                       np.sin(2 * np.pi * freq2 * time)) / 2
    ...
    def pulse_interference(self, time: float, signals: List[Signal]) -> Tuple[List[Signal], float]:
        """
...
class EnvironmentScenario:
    ...
    def open_area_scenario(self, signals: List[Signal], 
    ...
    def mountain_scenario(self, signals: List[Signal], mountain: Mountain) -> List[Signal]:
        """Mountain scenario - filter out obstructed satellites"""
        # Calculate obstruction angle
        obstruction_angle = np.arctan2(mountain.height, mountain.distance) * 180 / np.pi
        
        # Filter satellites below obstruction angle
        visible_signals = [sig for sig in signals if sig.elevation > obstruction_angle]
        
        return visible_signals
    
    def tunnel_scenario(self, signals: List[Signal], 
        ...
        if tunnel_state == TunnelState.IN_TUNNEL:
            ...
            error = 20.0 * np.exp(-time_counter / 10)  # Exponentially decreasing error
...
class GNSSTrainPositioningSimulator:
    ...
    def __init__(self):
        ...
        # Reference trajectory (simplified train path)
        self.reference_trajectory = self._generate_reference_trajectory()
        
    def _generate_reference_trajectory(self, num_points: int = 600) -> List[Coordinate]:
    ...
    def simulate_scenario(self, scenario: Scenario, 
        ...
        for t in range(duration):
            ...
            if scenario == Scenario.OPEN_AREA:
                ...
                signals = self.env_scenario.mountain_scenario(signals, mountain)
                int_error = 0.0
...
def create_figure_1_framework():
    ...
    for i, scenario in enumerate(scenarios):
        ...
        ax.add_patch(rect)
        ax.text(5, y_pos + 0.3, scenario, ha='center', va='center', fontsize=9)
    ...
    for i, interference in enumerate(interferences):
        ...
        rect = FancyBboxPatch((7, y_pos), 2, 0.6, boxstyle="round,pad=0.05",
                             edgecolor='darkred', facecolor='lightcoral', linewidth=1)
        ax.add_patch(rect)
        ax.text(8, y_pos + 0.3, interference, ha='center', va='center', fontsize=9)
...
def create_figure_2_hierarchy():
    ...
    rect1 = FancyBboxPatch((3, 8), 4, 0.8, boxstyle="round,pad=0.1",
                           edgecolor='blue', facecolor='lightblue', linewidth=2)
    ax.add_patch(rect1)
    ax.text(5, 8.4, 'Top Level', ha='center', va='center', fontsize=12, weight='bold')
    
    # Level 2: Main modules
    modules = ['GNSS Receiver', 'Position Solution', 'Evaluation']
    x_positions = [1.5, 4, 6.5]
    for i, (module, x) in enumerate(zip(modules, x_positions)):
        rect = FancyBboxPatch((x, 6), 2, 0.8, boxstyle="round,pad=0.1",
                             edgecolor='green', facecolor='lightgreen', linewidth=2)
        ax.add_patch(rect)
        ax.text(x + 1, 6.4, module, ha='center', va='center', fontsize=10, weight='bold')
    ...
    for i, sub in enumerate(sub_modules):
        y_pos = 4.5 - i * 1.2
        rect = FancyBboxPatch((0.5, y_pos), 1.5, 0.6, boxstyle="round,pad=0.05",
                             edgecolor='darkgreen', facecolor='lightgreen', linewidth=1)
        ax.add_patch(rect)
        ax.text(1.25, y_pos + 0.3, sub, ha='center', va='center', fontsize=9)
...
def create_petri_net_diagram(title: str, filename: str):
    ...
    ax.text(5, 7.5, title, ha='center', va='center', fontsize=14, weight='bold')
    ...
    for x, y, label in places:
        circle = Circle((x, y), 0.3, edgecolor='black', facecolor='white', linewidth=2)
        ax.add_patch(circle)
...
def create_figure_10_tunnel_errors(simulator: GNSSTrainPositioningSimulator):
    ...
    # Mark tunnel exit
    ax.axvline(x=300, color='red', linestyle='--', linewidth=2, label='Tunnel Exit')
    ...
    ax.set_title('Position Errors in Tunnel Scenario (Figure 10)', fontsize=14, weight='bold')
...
def create_table_1_colsets():
    ...
    colsets = [
        ...
        ("colset SIGNAL", "Record containing data from a single satellite (id, psr, psr_rate, x, y, z, vx, vy, vz, clk, azimuth, elevation, rate_clock)"),
        ("colset SIGNALlist", "List of SIGNAL records representing complete satellite data"),
        ("colset SIGNALLIST", "Product of INT * INT * SIGNALlist for multiple time points"),
        ("colset SCENARIO", "Environment scenarios: OpenArea | Mountain | Tunnel"),
        ("colset STATEINFE", "Interference states: AM | FM | Pulse | Normal"),
        ("colset Coordinate", "3D coordinate: REAL * REAL * REAL"),
        ("colset COORDINATE", "Coordinate with timestamp: INT * Coordinate"),
        ("colset DELTAPOSITION", "Positional difference: INT * Coordinate * REAL"),
        ...
        ("colset TUNNELSTATE", "Tunnel states: InTunnel | JustOut | OutTunnel"),
...
def create_table_2_variables():
    ...
    variables = [
        ...
        ("InterferenceData", "SIGNALLIST", "GNSS signal data under interferences"),
        ("StateInterference", "STATEINFE", "State of signal interference"),
        ("deltaPosition", "DELTAPOSITION", "Error between calculated and reference position"),
        ("bAM, bFM, bPulse", "BOOL", "Boolean variables for interference presence"),
        ("AMError, FMError, PulseError", "REAL", "Errors caused by interferences"),
        ("TrainPosition", "TUNNELSTATE", "State of train's position in tunnel"),
...
def create_table_4_interference_performance(simulator: GNSSTrainPositioningSimulator):
    ...
    plt.savefig('table_4_interference_performance.png', dpi=300, bbox_inches='tight')
    print("✓ Generated Table 4 visualization")
    plt.close()

def create_table_5_environment_performance(simulator: GNSSTrainPositioningSimulator):
    ...
    plt.title('Table 5: Positioning Performance Under Environment Scenarios', 
             fontsize=14, weight='bold', pad=20)
    plt.savefig('table_5_environment_performance.png', dpi=300, bbox_inches='tight')
...
def create_automaton_diagram():
    ...
    for x, y, label in states:
        circle = Circle((x, y), 0.5, edgecolor='blue', facecolor='lightblue', linewidth=2)
        ax.add_patch(circle)
    ...
    for (x1, y1), (x2, y2), label in transitions:
        ...
        ax.text(mid_x, mid_y + 0.2, label, ha='center', fontsize=8, 
               style='italic', bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
...
def create_comparison_plots(simulator: GNSSTrainPositioningSimulator):
    ...
    plt.tight_layout()
    plt.savefig('comparison_interference_errors.png', dpi=300, bbox_inches='tight')
    ...
    plt.suptitle('Position Errors Under Different Environment Scenarios', 
                fontsize=14, weight='bold')
    plt.tight_layout()
    plt.savefig('comparison_environment_errors.png', dpi=300, bbox_inches='tight')
...
def main():
    ...
    create_petri_net_diagram('Figure 3: Top-Level CPN Model', 'figure_3_top_level.png')
    create_petri_net_diagram('Figure 4: GNSS Receiver Module', 'figure_4_gnss_receiver.png')
    create_petri_net_diagram('Figure 5: Open Area Submodule', 'figure_5_open_area.png')
    create_petri_net_diagram('Figure 6: Mountain Submodule', 'figure_6_mountain.png')
    create_petri_net_diagram('Figure 7: Tunnel Submodule', 'figure_7_tunnel.png')
    create_petri_net_diagram('Figure 8: Position Solution Module', 'figure_8_position_solution.png')
    create_petri_net_diagram('Figure 9: Evaluation Module', 'figure_9_evaluation.png')
