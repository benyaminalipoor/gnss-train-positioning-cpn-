...
def least_squares_positioning(
    ...
    # Pre-allocate arrays outside loop for performance
    n_satellites = len(signals)
    H = np.zeros((n_satellites, 4))
    residuals = np.zeros(n_satellites)
    
    # Pre-convert satellite positions to numpy array
    sat_positions = np.array([[signal.x, signal.y, signal.z] for signal in signals])
    
    # Iterative least squares
    HTH = None  # Cache H^T H for DOP calculation
    for iteration in range(max_iterations):
        # Reset arrays instead of reallocating
        H.fill(0)
        residuals.fill(0)
        
        # Vectorized geometric range calculation
        ...
        for i in range(n_satellites):
            ...
            if geometric_ranges[i] > 0:
                unit_vector = diff[i] / geometric_ranges[i]
            else:
            ...
            predicted_range = geometric_ranges[i] + state[3]
            residuals[i] = signals[i].psr - predicted_range
    ...
    # Calculate DOP (Dilution of Precision) values - reuse HTH if available
    try:
        if HTH is None:
            HTH = H.T @ H
        Q = np.linalg.inv(HTH)
